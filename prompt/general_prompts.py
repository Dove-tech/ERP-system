...
class PromptModelHub:
    ...
    def __init__(self, system_prompt, model_name="generic"):
    ...
    def post_process_tool_selection_result(self, answer_str, tools: List[Tool]) -> Tool:
        ...
        structured_action = StructuredOutputParser.parse_tool_action(answer_str)
        if structured_action:
            structured_action = structured_action.replace(",", "")
            structured_action = structured_action.replace("[", "")
            structured_action = structured_action.replace("]", "")
            if structured_action.lower() == "none":
                return self.stop_label
            if structured_action in tools_model2human:
                return tools_model2human[structured_action]
            if structured_action in tools_human2model:
                return tools_human2model[structured_action]
    ...
    def gen_human_feedback_intent_prompt(self, feedback: str, tool_name: str, tool_params) -> str:
    ...
    def post_process_human_feedback_intent_result(self, answer: str):
        return StructuredOutputParser.parse_intent(answer)
