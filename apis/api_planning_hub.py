...
class ApiPlanningHub:
    ...
    def _recognize_human_intent(self, human_feedback, tool, tool_param):
        ...
        # 使用大模型进行语义理解
        try:
            prompt = self.api_selection_hub.PromptModelHub.gen_human_feedback_intent_prompt(
                human_feedback,
                tool.name_for_human,
                tool_param,
            )
            ...
            intent_result = self.api_selection_hub.PromptModelHub.post_process_human_feedback_intent_result(response)
            logger.info(f"人类反馈意图识别结果: {intent_result}")
            return intent_result["intent"]
        except Exception as e:
            logger.error(f"结构化人类反馈意图识别失败，回退到旧提示词: {e}\n{traceback.format_exc()}")
